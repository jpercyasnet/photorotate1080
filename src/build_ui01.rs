...
pub fn build_ui(application: &gtk::Application) {
    ...
    let wtitle = format!("Photo Rotate and Convert to 1080 Rust GTK4 version: {}.{}.{}",gtk::get_major_version(), gtk::get_minor_version(), gtk::get_micro_version());
...
// -------- second tab
    ...
    gtk::WidgetExt::set_widget_name(&listgroup_button, "label1");
...
//----------------- list  button start -----------------------------------
    ...
    listgroup_button.connect_clicked(glib::clone!(@weak directory_combobox, @weak messageval_label, @weak from_entry, @weak to_entry, @weak iconsize_entry, @weak progress_progressbar, @weak icon_view => move|_| {
        ...
        if let Some(cur_dir) = directory_combobox.get_active_text() {
//            if let Some(input_text) = from_entry.get_text() { 
            if from_entry.get_text_length() == 0 {
                messageval_label.set_markup("<span color=\"#FF000000\">********* List: ERROR GETTING TEXT FROM FROM SIZE ENTRY **********</span>");
            } else {
                let input_text = from_entry.get_text();
...
//                messageval_label.set_markup("<span color=\"#FF000000\">********* List: ERROR GETTING TEXT FROM FROM SIZE ENTRY **********</span>");
            ...
            if badsize_int == 0 {
                badsize_int = 1;
//                if let Some(inputto_text) = to_entry.get_text() { 
                if to_entry.get_text_length() == 0 {
                    messageval_label.set_markup("<span color=\"#FF000000\">********* List: ERROR GETTING TEXT FROM TO SIZE ENTRY **********</span>");
                } else {
                    let inputto_text = to_entry.get_text();
...
//                    messageval_label.set_markup("<span color=\"#FF000000\">********* List: ERROR GETTING TEXT FROM TO SIZE ENTRY **********</span>");
                ...
                if badsize_int == 0 {
                    if to_int1 < from_int1 {
                        ...
                        badsize_int = 1;
//                        if let Some(inputic_text) = iconsize_entry.get_text() { 
                        if iconsize_entry.get_text_length() != 0 { 
                            let inputic_text = iconsize_entry.get_text();
...
//----------------- rotate all button start -----------------------------------
    ...
    rotateall_button.connect_clicked(glib::clone!(@weak directory_combobox, @weak messageval_label, @weak tree_view, @weak progress_progressbar => move|_| {
        ...
        let treemodel = tree_view.get_model();
        if treemodel == None {
            ...
            if let Some(cur_dir) = directory_combobox.get_active_text() {
                ...
                let mut valid = true;
                let validval = treemodeluw.get_iter_first().unwrap();
                let mut numrow = 0;
                let numchildren = treemodeluw.iter_n_children(None);
                let mut msgvar = format!(" ");
                let mut numprocess = 0;
                while valid {
                      let treeval = treemodeluw.get_value(&validval,1);
                      let filenameval = treemodeluw.get_value(&validval,0);
                          ...
                          if orient_int > 0 {
                              if (orient_int == 3) | 
                                 ...
                                 (orient_int == 8) {
                                  ...
                                  let filestart = 9;
                                  let filenamex = filenamestr.get(filestart..fileend).unwrap();
            ...
            if let Some(cur_dir) = directory_combobox.get_active_text() {
                ...
                if icon_selectpath.len() > 0 {
                    ...
                    for path in &icon_selectpath {
                         numrot = numrot + 1;
                         let iconiter = iconmodeluw.get_iter(&path).unwrap();
...
//----------------- directory output button start -----------------------------------
    ...
    dirout_button.connect_clicked(glib::clone!(@weak window, @weak dirout_combobox, @weak messageval_label => move|_| {
        ...
        dialog.connect_response(move |d: &FileChooserDialog, response: gtk::ResponseType| {
            ...
            if messageval_label.get_text() == "getting directory output" {
...
//----------------- copy convert button start -----------------------------------
    ...
    copy_button.connect_clicked(glib::clone!(@weak directory_combobox, @weak dirout_combobox, @weak messageval_label, @weak tree_view2, @weak progress_progressbar => move|_| {
        ...
        let treemodel = tree_view2.get_model();
        if treemodel == None {
             messageval_label.set_markup("<span color=\"#FF000000\">********* Copy convert: ERROR NOTHING IN LIST **********</span>");
        } else {
            if let Some(from_dir) = directory_combobox.get_active_text() {
                if let Some(to_dir) = dirout_combobox.get_active_text() {
                    ...
                    if numfiles > 0 {
                         ...
                         while valid {
                             ...
                             if valid & (numprocess < 4) {
                                      Command::new("convert")
                                             .arg(&s1_param)
 //                                            .arg("-resize 1920x1080 -background black -gravity center -extent 1920x1080")
                                             .arg("-resize")
                                             .arg("1920x1080")
                                             .arg("-background")
                                             .arg("black")
                         ...
                         let msgstr = format!("Number of files copied: {} {}", numrot, msgvar);
                         messageval_label.set_text(&msgstr);
                    }
                } else {
